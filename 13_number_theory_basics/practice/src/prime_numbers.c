...
int64_t gcd(int64_t a, int64_t b){ // Euclidean algorithm
    if (b == 0) {
        return a;
    }
    return gcd(b, a%b);
...
int main()
{
    ...
    printf("Onde iteration is generate random number in range 0-%d and check is it prime or not\n", NUMBER_LIMITS);
    ...
    printf("Onde iteration is generate random number in range 0-%ld and check is it prime or not\n", (int64_t)NUMBER_LIMITS * NUMBER_LIMITS * NUMBER_LIMITS);
    for (int i = 0; i  < N_ITERATIONS; ++i) {
