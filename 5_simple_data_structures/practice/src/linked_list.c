...
void insert_before_value(linked_list_t* list, uint32_t value_to_insert_before, uint32_t value) {
    linked_list_node_t* node_to_insert_before = get_previous_node_by_value(list, value_to_insert_before);

    if(node_to_insert_before) {
        linked_list_node_t* inserted_node = (linked_list_node_t*)malloc(sizeof(linked_list_node_t));

        inserted_node->value = value;
        inserted_node->next = node_to_insert_before->next;
        node_to_insert_before->next = inserted_node;
    } else {
